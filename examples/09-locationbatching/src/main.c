...
static void loc_batching_cb(size_t count, qapi_Location_t* locs)
{
	TRACE("batching callback (%d locations)\r\n", count);

	for(size_t i=0; i<count; i++) {
		if(locs[i].size != sizeof(qapi_Location_t)) {
			TRACE("param size missmatch\r\n");
			return;
		}
		qapi_Location_t loc = locs[i];
...
int dam_app_start(void)
{
	...
	cbs.batchingCb = loc_batching_cb;
